...
- `--save-images auto`：BIN 输入默认只把没有检测到人脸的帧保存到 `missed/`，不再保存每一张成功帧。

输出 AVI 会包含帧号、人脸数、检测框和关键点；`missed/` 中只保留漏检帧。`detections.csv` 会记录所有已处理帧的 `frame_index`、原始温度最小值、最大值和平均值。
